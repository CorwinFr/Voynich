#!/usr/bin/env python3
"""
Build the French narrative PDF for LinkedIn.
Usage: python build_fr.py [output_path]
"""
import sys, os
...
from content_fr import build_story_fr
    ...
    def _draw_page_extras(self, page_num, total):
        ...
        if page_num > 1:
            ...
            self.drawRightString(W - MARGIN_R, H - 14.5 * mm,
                "Le Manuscrit de Voynich : Journal d'un D\xe9cryptage")
        self.restoreState()


def main():
    output = sys.argv[1] if len(sys.argv) > 1 else "Voynich_Journal_Decryptage_FR.pdf"

    doc = SimpleDocTemplate(output, pagesize=A4,
        leftMargin=MARGIN_L, rightMargin=MARGIN_R,
        topMargin=22 * mm, bottomMargin=22 * mm)

    story = build_story_fr()
